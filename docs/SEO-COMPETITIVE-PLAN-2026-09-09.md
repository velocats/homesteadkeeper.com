...
| Add accountable guide authorship and review dates | `src/data/guides.ts`, guide template, About page as appropriate | Completed locally: visible Quiet Tools Support Team attribution, Colorado location, and September 9, 2026 review date match the structured data and About page. |
| Add responsive hero image variants | Screenshot component and assets | Completed locally: 480, 800, and 1200 pixel WebP candidates, plus the 2048 pixel original, use accurate dimensions and `sizes`; the eager hero retains high fetch priority. Mobile Lighthouse results are recorded in the implementation log. |
...
First-pass status: removed build-time `lastmod`; verified all six original video publication timestamps from their YouTube watch pages; added those timestamps to tutorial markup and reused them in the checklist guide; removed watch-page `contentUrl` values. Added visible guide attribution to the Quiet Tools Support Team in Colorado, United States, with a genuine September 9, 2026 review date reflected in Article structured data and the About page. Added and measured responsive homepage hero candidates. Built-output checks passed. External Rich Results Test and production verification remain pending.
...
- [ ] Verify every product claim against the current release before publication. A claim matrix and release checklist now live in `docs/SEO-CLAIMS-REVIEW-2026-09-09.md`; the installed-release checks still require owner/app access.
...
| 2026-09-09 | Added accountable guide attribution to the Quiet Tools Support Team in Colorado, United States, with a visible review date and matching structured data; documented the team on About | Build passed (65 pages). Built-output checks passed across all 10 guides for visible team name, location, review date, Article `dateModified`, author entity link, and About-page identity. Not deployed. |
| 2026-09-09 | Added 480, 800, and 1200 pixel WebP `srcset` candidates for the homepage hero and the repeated Homestead Hub showcase; retained the original for lightbox expansion | Matching Lighthouse 12.8.2 simulated-mobile runs against the local Astro server: hero request fell from 269 KiB to 61 KiB, LCP improved from 9.3 s to 7.5 s, performance score moved from 62 to 63, and CLS stayed at 0. These are lab results, not production field data; production Lighthouse and CrUX verification remain pending. |
